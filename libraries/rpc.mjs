...
export async function getRootDomain(baseUrl = "") {
  if (rootDomain) {
    return rootDomain;
  } else {
    rootDomain = await getJson(baseUrl + "/common/rootdomain");
    return rootDomain;
  }
}

export async function siblingUrl(baseUrl, service, path) {
  const rootDomain = await getRootDomain(baseUrl);
...
const messageRouting = [
  ...
  },
  {
    ...
    route: async (payload) => {
      ...
      try {
        const contractName = await getContractName();
        const query = registeredQueries.find((q) => q.id === identifier);
        if (query) {
          try {
            const msgMetadata = { sender };
            responsePayload.response = await query.exec(params, msgMetadata);
          } catch (e) {
            responsePayload.errors.push(e);
          }
        } else {
  ...
  },
  {
    ...
    route: (payload) => {
      ...
      return callbackId ? executePromise(callbackId, trace, errors) : false;
    }
  }
];
